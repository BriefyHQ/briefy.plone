...
def scale_info(name, context, s3_path, field_name, width, height):
    """Return scale info."""
    url = u'{0}/@@images/{1}/{2}'.format(context.absolute_url(), field_name, name)

    if s3_path:
        url = generate_url(s3_path, width, height)
...
def get_scales(context, field, width, height):
    ...
    for name, scale_width, scale_height in get_scale_infos():
        ...
        scales[name] = scale_info(name, context, s3_path, field_name, width, height)
